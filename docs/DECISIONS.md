...
removes gh's own value (`!gh auth git-credential`, or with gh's path) and the empty
reset directly before it, and nothing else. A composite helper, another CLI's
helper, and any other empty reset all stay, because the undo would never bring
them back.
